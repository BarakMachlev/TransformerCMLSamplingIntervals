...
sampling_intervals = [10, 60, 300, 900]

for sampling_interval_in_sec in sampling_intervals:

    print("===================================================")
    print(f"Running experiment for {sampling_interval_in_sec} sec")
    print("===================================================")

    samples_type = "instantaneous"

    base_output_dir = (f"/home/lucy3/BarakMachlev/Thesis/Article_Results/UT/openMRG_DataSet/Robustness/Train{sampling_interval_in_sec}sec/Iter_3")
    ...
    # Make sure every physical link appears exactly 1 time
    from collections import Counter
    counts = Counter([lnk.link_index for lnk in dataset.link_set.link_list])
    assert all(v == 1 for v in counts.values()), "for all physical links 1 protocol index which does not matter"

    # Make sure no leakage
    assert set(val_phys).isdisjoint(set(train_phys)), "Leakage between train and validation"

    # sanity: expect 1 variant per physical link
    print(f"Total items: {len(dataset.link_set.link_list)}")
    print(f"Train items: {len(train_indices)}  (expected ~ {64})")
    print(f"Val items:   {len(val_indices)}    (expected ~ {16})")
    ...
    if os.path.exists(model_path):
        ...
        loss_detection = 0
        with torch.no_grad():
            for rain_rate, attenuation, metadata in data_loader:
        ...
        for epoch in tqdm(range(n_epochs)): # Repeat the whole training process again
            am.clear()
            for rain_rate, attenuation, metadata in data_loader: # for loop for true batches - each batch has batch_size links
            #for rain_rate, rsl, tsl, metadata, protocol_id in data_loader:    
                m_step = math.floor(rain_rate.shape[1] / window_size)
            ...
            am_val.clear()
            with torch.no_grad():
                for rain_rate_v, attenuation_v, metadata_v in val_loader:
