...
from pynncml.single_cml_methods.rain_estimation import one_step_dynamic_baseline
...
base_output_dir = "./Article_Results/UT/openMRG_DataSet/DynamicBaseline"

batch_size = 16
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

dynamic_baseline_model = one_step_dynamic_baseline(
...
for sampling_interval_in_sec in sampling_intervals:
    ...
    print("===================================================")
    print(f"Running Dynamic Baseline for {sampling_interval_in_sec} sec")
    print("===================================================")
    ...
    with torch.no_grad():
        ...
        for rain_rate, attenuation, metadata, protocol_id in tqdm(full_loader):
            ...
            for link_idx in range(att_token.shape[0]):
                ...
                rain_hat_link, _ = dynamic_baseline_model(
                    input_att,
                    meta
                )
            ...
            rain_detection = (rain_hat > 0.1).float()
    ...
    results_path = os.path.join(
        output_dir,
        "Estimation_Results_DynamicBaseline.txt"
    )
...
summary_path = os.path.join(base_output_dir, "summary_dynamic_baseline.csv")
...
print("Finished Dynamic Baseline evaluation")
